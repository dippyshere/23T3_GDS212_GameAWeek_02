    ...
    public class WakeGenerator : MonoBehaviour
    {
        ...
        private IEnumerator GenerateWake(Wake wake)
        {
            for (var i = 0; i < 2; i++)
            {
                ...
                var wakeLoading = Instantiate(wakePrefab, Vector3.zero, Quaternion.Euler(90f, 0, 0));

                yield return wakeLoading;

                _lrs.Add(wakeLoading);
                wakeLoading.TryGetComponent(out wl.lineRenderer);
                wakeLoading.hideFlags = HideFlags.HideAndDontSave;
